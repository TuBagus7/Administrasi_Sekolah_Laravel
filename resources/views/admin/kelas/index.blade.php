...
@section('script')
  <script>
    function getCreateKelas(){
      ...
      `);
      $('#nama_kelas').val('');
      // Hapus bagian form_paket karena sudah tidak pakai paket
      // $('#form_paket').html('');
      $('#guru_id').val('');
    }
    ...
    function getEditKelas(id){
      var parent = id;
      // Hapus form_paket, karena tidak dipakai
      $.ajax({
        type:"GET",
        data:"id="+parent,
        dataType:"JSON",
        ...
        success:function(result){
          if(result){
            $.each(result,function(index, val){
              $("#judul").text('Edit Data Kelas ' + val.nama_kelas);
              $('#id').val(val.id);
              $('#form_nama').html('');
              $('#form_nama').html(`
                <label for="nama_kelas">Nama Kelas</label>
